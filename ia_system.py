...
class IASystem:
    ...
    def setup_commands(self):
        ...
        @self.bot.command(name="IA", aliases=["ia"])
...
Ahora sí, continúa con lo que pide el usuario: """ + peticion
                        ...
                        try:
                            async with session.post(url, json=payload, headers=headers) as response:
                                if response.status == 200:
                                    ...
                                    await ctx.send("❌ Error: La petición fue rechazada por la API (contenido inapropiado o muy largo)")
                                
                                elif response.status == 403:
                                    await ctx.send("❌ Error: La API key no es válida o ha expirado")
                                
                                else:
                                    await ctx.send(f"❌ Error del servidor: HTTP {response.status}")
                ...
                await ctx.send("❌ Ocurrió un error procesando tu petición")
