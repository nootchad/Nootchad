...
class CodesSystem:
        ...
        try:
            if Path(self.codes_file).exists():
                with open(self.codes_file, 'r', encoding='utf-8') as f:
                    ...
                    logger.info(f"<a:verify2:1418486831993061497> Cargados {len(self.codes)} códigos promocionales")
            else:
                self.codes = {}
        ...
        try:
            if Path(self.codes_usage_file).exists():
                with open(self.codes_usage_file, 'r', encoding='utf-8') as f:
                    ...
                    logger.info(f"<a:verify2:1418486831993061497> Cargados registros de uso de {len(self.codes_usage)} códigos")
            else:
                self.codes_usage = {}
    ...
    def redeem_code(self, user_id: str, username: str, code: str) -> dict:
        ...
        logger.info(f"<a:verify2:1418486831993061497> Usuario {username} ({user_id}) canjeó código {code}")
