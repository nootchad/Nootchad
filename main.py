...
from friend_browser_improved import FriendRequestBrowserImproved
        ...
        # Crear embed inicial
        initial_embed = discord.Embed(
            title="🔄 Friend Requests con REINICIO COMPLETO",
            description=f"Procesando **{cantidad}** solicitudes con navegador COMPLETAMENTE NUEVO para cada cookie para usuario ID: `{user_id}`",
            color=0x3366ff
        )
        initial_embed.add_field(name="👤 Usuario Objetivo", value=f"`{user_id}`", inline=True)
        initial_embed.add_field(name="🔄 Método", value="NAVEGADOR NUEVO por cookie", inline=True)
        initial_embed.add_field(name="⚡ Ventaja", value="Logout 100% garantizado", inline=True)
        initial_embed.add_field(name="⏳ Estado", value="Iniciando proceso...", inline=False)
        
        message = await interaction.followup.send(embed=initial_embed, ephemeral=True)
        
        # Ejecutar el proceso de friend requests
        logger.info(f"🚀 Owner {interaction.user.name} inició friendrestart para usuario {user_id} con {cantidad} cookies")
        
        resultado = await friend_system.process_friend_requests_with_restart(user_id, cantidad)
        ...
        if resultado["status"] == "completed":
            ...
            if results["exitosas"] > 0:
                color = 0x00ff88  # Verde si hay éxitos
                title = "✅ Friend Requests Completados (REINICIO COMPLETO)"
            elif results["ya_amigos"] > 0:
                color = 0xffaa00  # Amarillo si ya son amigos
                title = "👥 Friend Requests Procesados (REINICIO COMPLETO)"
            else:
                color = 0xff0000  # Rojo si todas fallaron
                title = "❌ Friend Requests Fallidos (REINICIO COMPLETO)"
            
            final_embed = discord.Embed(
                title=title,
                description=f"Proceso completado para el usuario ID: `{user_id}` con navegador COMPLETAMENTE NUEVO para cada cookie",
                color=color
            )
            
            final_embed.add_field(name="👤 Usuario Objetivo", value=f"`{user_id}`", inline=True)
            final_embed.add_field(name="📊 Total Procesadas", value=f"{results['total_procesadas']}", inline=True)
            final_embed.add_field(name="🔄 Método", value="Navegador nuevo x cookie", inline=True)
            
            final_embed.add_field(name="✅ Exitosas", value=f"{results['exitosas']}", inline=True)
            final_embed.add_field(name="❌ Fallidas", value=f"{results['fallidas']}", inline=True)
            final_embed.add_field(name="👥 Ya Amigos", value=f"{results['ya_amigos']}", inline=True)
            
            # Agregar ventajas del método
            final_embed.add_field(
                name="🚀 Ventajas del REINICIO COMPLETO:",
                value="• ✅ Navegador 100% nuevo para cada cookie\n• ✅ Logout garantizado sin residuos\n• ✅ Máxima separación de sesiones\n• ✅ Proceso más limpio y confiable",
                inline=False
            )
            
            # Agregar errores si los hay (limitado)
            if results["errores"]:
                errores_text = "\n".join(results["errores"][:3])
                if len(results["errores"]) > 3:
                    errores_text += f"\n... y {len(results['errores']) - 3} errores más"
                final_embed.add_field(
                    name="⚠️ Errores:",
                    value=f"```{errores_text}```",
                    inline=False
            ...
            if results["exitosas"] > 0:
                final_embed.add_field(
                    name="🎉 Resultado:",
                    value=f"Se enviaron {results['exitosas']} solicitudes de amistad exitosamente con navegador completamente nuevo para cada cookie.",
                    inline=False
                )
            elif results["ya_amigos"] > 0:
                final_embed.add_field(
            ...
            # Error en el proceso
            final_embed = discord.Embed(
                title="❌ Error en Friend Requests (REINICIO COMPLETO)",
                description=f"Ocurrió un error durante el proceso: {resultado['message']}",
                color=0xff0000
            )
            final_embed.add_field(name="💡 Sugerencia", value="Verifica las cookies y la conexión a internet", inline=False)
        
        final_embed.set_footer(text=f"Comando ejecutado por {interaction.user.name} • REINICIO COMPLETO del navegador")
        
        await message.edit(embed=final_embed)
        
        logger.info(f"Owner {interaction.user.name} completó friendrestart para usuario ID: {user_id}")
        
    except Exception as e:
        logger.error(f"Error en comando friendrestart: {e}")
        error_embed = discord.Embed(
            title="❌ Error Interno",
            description=f"Ocurrió un error durante el envío de las solicitudes de amistad con reinicio completo.",
            color=0xff0000
        )
        error_embed.add_field(name="🐛 Error", value=f"```{str(e)[:150]}{'...' if len(str(e)) > 150 else ''}```", inline=False)
        error_embed.add_field(name="💡 Sugerencia", value="Verifica las cookies y la configuración del navegador", inline=False)
        await interaction.followup.send(embed=error_embed, ephemeral=True)
