...
async def on_ready():
    ...
    # Setup IA system
    global ia_system_global
    try:
        from ia_system import setup_ia_commands
        
        ia_system_global = setup_ia_commands(bot)
...
Genera una imagen que sea visualmente impactante y que capture perfectamente la esencia de lo solicitado."""
    ...
    # Verificar si el mensaje comienza con !IA
    elif message.content.startswith('!IA ') or message.content.startswith('!ia '):
        user_id = str(message.author.id)
        username = f"{message.author.name}#{message.author.discriminator}"
        ...
        try:
            # Extraer la consulta después de "!IA " o "!ia "
            consulta = message.content[4:].strip()  # Remover "!IA " o "!ia " del inicio
            
            # Verificar que la consulta no esté vacía
            if not consulta:
                await message.reply("❌ Debes proporcionar una consulta para la IA.\n**Ejemplo:** `!IA ¿Cómo hacer un script de teletransporte en Roblox?`", mention_author=False)
                return
            
            # Verificar longitud de la consulta
            if len(consulta) > 500:
                await message.reply(f"❌ La consulta es muy larga ({len(consulta)} caracteres). El límite es 500 caracteres.", mention_author=False)
                return
            
            # Usar la instancia global del sistema de IA
            if not ia_system_global:
                await message.reply("❌ Sistema de IA no disponible.", mention_author=False)
                return
            
            # Crear el prompt personalizado igual que en /scripts
            system_prompt = """A partir de ahora, debes seguir estrictamente las siguientes reglas:
...
Ahora sí, continúa con lo que pide el usuario: """ + consulta
            ...
            if not gemini_api_key:
                await message.reply("❌ API key de Gemini no está configurada.", mention_author=False)
                return
                        ...
                        if response.status == 200:
                            ...
                            if "candidates" in data and len(data["candidates"]) > 0:
                                ...
                                if "content" in candidate and "parts" in candidate["content"]:
                                    ...
                                    await loading_message.edit(embed=response_embed)
                                    
                                    # Log del uso
                                    logger.info(f"Usuario {username} (ID: {user_id}) usó !IA: {consulta[:50]}{'...' if len(consulta) > 50 else ''}")
                                    ...
                                    await loading_message.edit(content="❌ Error: Respuesta inválida de la API de IA")
                            else:
                                await loading_message.edit(content="❌ Error: No se recibió respuesta válida de la API de IA")
                        else:
                            await loading_message.edit(content=f"❌ Error HTTP {response.status} de la API de IA")
                
                except Exception as e:
                    logger.error(f"Error en !IA: {e}")
                    await loading_message.edit(content="❌ Error conectando con RbxServers-v1 IA")
